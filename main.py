...
class TaxCalculationRequest(BaseModel):
    ...
    standard_deduction: float = 75000  # Updated to ₹75,000 for FY 2025-26
    edu_loan_interest: float = 0
    donations: float = 0
    # New specialized deductions (Old Regime only)
    section80U: str = "none"  # "none", "standard", "severe"
    royaltyIncome: float = 0  # 80QQB/RRB combined
    politicalDonation: float = 0  # 80GGC
    evLoanInterest: float = 0  # 80EEB
...
def calculate_old_regime_tax(income: float, total_deductions: float = 0) -> float:
    ...
    # FY 2025-26 Old Regime slabs (unchanged from previous year)
...
def calculate_advanced_deductions(request: TaxCalculationRequest) -> float:
    """Calculate advanced deductions with precise rules (Old Regime only)."""
    total_advanced = 0.0
    
    # Section 80U: Disability Deduction
    if request.section80U == "standard":
        total_advanced += 75000  # Standard Disability (40%+)
    elif request.section80U == "severe":
        total_advanced += 125000  # Severe Disability (80%+)
    
    # Section 80QQB/RRB: Royalty Income (min of actual or ₹3,00,000)
    royalty_deduction = min(request.royaltyIncome, 300000)
    total_advanced += royalty_deduction
    
    # Section 80GGC: Political Donations (100% of input, capped at Gross Total Income)
    # Ensure it doesn't exceed total income
    political_deduction = min(request.politicalDonation, request.income)
    total_advanced += political_deduction
    
    # Section 80EEB: EV Loan Interest (min of actual or ₹1,50,000)
    ev_deduction = min(request.evLoanInterest, 150000)
    total_advanced += ev_deduction
    
    return total_advanced

def calculate_financial_fitness_score(deductions: Dict, income: float) -> int:
    """Calculate Financial Fitness Score (0-100) based on tax optimization."""
    
    # Maximum possible deductions across all categories
    max_deductions = {
        "section80C": 150000,
        "section80D": 25000,  # Non-senior citizen
        "hra": 200000,  # Reasonable maximum
        "home_loan_interest": 200000,
        "standard_deduction": 75000,
        "edu_loan_interest": 150000,
        "donations": 100000,
        "advanced_deductions": 775000  # Sum of all advanced: 75K+125K+300K+unlimited+150K
    }
    
    # Calculate utilization percentage
    total_utilized = sum(deductions.values())
    total_possible = sum(max_deductions.values())
    
    # Cap at income to avoid >100%
    total_possible = min(total_possible, income)
    
    score = min(100, max(0, round((total_utilized / total_possible) * 100)))
    return score

def generate_investment_suggestions(deductions: Dict, income: float, savings: float) -> List[str]:
    """Generate wealth creation suggestions based on current savings and deductions."""
    suggestions = []
    
    if savings > 0:
        # ELSS suggestion (part of 80C)
        if deductions["section80C"] < 150000:
            remaining_80c = 150000 - deductions["section80C"]
            suggestions.append(f"💰 Invest ₹{remaining_80c:,.0f} in ELSS funds (Section 80C) to build wealth while saving ₹{round(savings * 0.3):,.0f} more in taxes.")
        
        # NPS suggestion (80CCD(1B))
        suggestions.append(f"🏛️ Consider investing ₹50,000 in NPS (Section 80CCD(1B)) for retirement security and additional ₹{round(savings * 0.2):,.0f} tax savings.")
        
        # Wealth building path
        annual_savings = abs(savings)
        suggestions.append(f"📈 Your annual tax savings of ₹{annual_savings:,.0f} can grow to ₹{annual_savings * 20:,.0f} over 20 years if invested wisely.")
    
    return suggestions

def calculate_new_regime_tax(income: float, standard_deduction: float = 75000) -> float:
    """Calculate tax under New Regime for FY 2025-26 (Budget 2025-2026).

    - Applies standard deduction only (default ₹75,000 - increased from ₹50,000).
    - New Slabs: 0–4L:0%, 4–8L:5%, 8–12L:10%, 12–16L:15%, 16–20L:20%, 20–24L:25%, 24L+:30%
    - Section 87A rebate: ₹60,000 for taxable income up to ₹12,00,000
    ...
    # FY 2025-26 New Regime slabs (Budget 2025-2026)
    slabs = np.array([0, 400000, 800000, 1200000, 1600000, 2000000, 2400000])
    rates = np.array([0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30])
    ...
    # Section 87A rebate: If taxable income <= ₹12,00,000, tax liability is zero (before cess)
...
async def calculate_tax(request: TaxCalculationRequest):
    ...
    try:
        ...
        for field_name, val in [
            ...
            ("royaltyIncome", request.royaltyIncome),
            ("politicalDonation", request.politicalDonation),
            ("evLoanInterest", request.evLoanInterest),
        ]:
            try:
        ...
        # Validate section80U enum
        if request.section80U not in ["none", "standard", "severe"]:
            raise HTTPException(status_code=400, detail="Invalid value for section80U. Must be 'none', 'standard', or 'severe'")
        
        # Calculate advanced deductions with precise rules
        advanced_deductions = calculate_advanced_deductions(request)
        
        # Log advanced deductions breakdown
        logging.info(f"Advanced deductions breakdown: section80U={request.section80U}, royaltyIncome={request.royaltyIncome}, politicalDonation={request.politicalDonation}, evLoanInterest={request.evLoanInterest}, total_advanced={advanced_deductions}")
        ...
        deductions = {
            ...
            "edu_loan_interest": request.edu_loan_interest,
            "donations": request.donations,
            # Advanced deductions (calculated with precise rules)
        ...
        # Calculate financial fitness score and investment suggestions
        fitness_score = calculate_financial_fitness_score(deductions, income)
        investment_suggestions = generate_investment_suggestions(deductions, income, savings)
        ...
        result = {
            ...
            "optimization_score": fitness_score,
...
async def export_pdf(request: ExportRequest):
    ...
    try:
        ...
        logging.error(f"Error in /export/pdf: {e}\n{traceback.format_exc()}")
...
async def export_excel(request: ExportRequest):
    ...
    try:
        ...
        logging.error(f"Error in /export/excel: {e}\n{traceback.format_exc()}")
